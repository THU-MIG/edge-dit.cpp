// Vendored from SageAttention (Apache-2.0). thu-ml/SageAttention csrc/qattn/qk_int_sv_f16_cuda_sm80.cu
// Only the __global__ kernel template is kept; torch launchers replaced by raw-pointer host code in ed_cuda_sage_attn.cu
#pragma once
#include <cuda_fp16.h>
#include <cuda_bf16.h>
...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
        typename DTypeSVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_mean=false, bool smooth_q=false>
__global__ void qk_int_sv_f16_attn_kernel(int8_t *__restrict__ Q, int8_t *__restrict__ K, half *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
                      ...
                      float sm_scale,
                      // smooth-Q rank-1 compensation: per-(batch, key) bias c[j] = qm·K[j] that
                      // recovers the per-key term removed by per-token Q-mean subtraction. Only
                      // read when smooth_q; layout [batch][kv_padded] with row stride stride_bz_bias.
                      const float *__restrict__ QK_bias = nullptr,
                      const uint32_t stride_bz_bias = 0)
{
  // compile time check
...
#pragma unroll
        for (uint32_t k = 0; k < 8; k++)
        {
          if constexpr (smooth_q)
          {
            // dequantize now so the rank-1 bias (true QK units) can be added
            // here; update_mdo below is then fed original_sm_scale.
    ...
    // smooth-Q per-key compensation (this block's keys start at K_idx_lane_base,
    // which is still the current block's base -- it is incremented just below).
    if constexpr (smooth_q)
    {
      add_smooth_q_bias<num_tiles_q, num_tiles_k>(RS_f32, QK_bias + batch_id * stride_bz_bias, K_idx_lane_base, kv_len);
    }
    ...
    if constexpr (std::is_same<DTypeSVAccum, float>::value)
    {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, false, false, false>(RS_f32, RO, m, d, smooth_q ? original_sm_scale : sm_scale);
    }
    else if constexpr (std::is_same<DTypeSVAccum, half>::value)
    {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, true, false, false>(RS_f32, RO, m, d, smooth_q ? original_sm_scale : sm_scale);
    }
