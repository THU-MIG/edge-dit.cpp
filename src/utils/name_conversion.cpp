...
std::string convert_diffusers_qwen_image_vae_to_wan(std::string name) {
    static const std::vector<std::pair<std::string, std::string>> direct_map = {
        ...
        {"encoder.down_blocks.", "encoder.downsamples."},
    };

    replace_with_prefix_map(name, direct_map);
    ...
    for (const char* scope : {"encoder", "decoder"}) {
        const std::string mid_prefix = std::string(scope) + ".mid_block.";
        if (starts_with(name, mid_prefix + "resnets.0.")) {
            name = std::string(scope) + ".middle.0." + name.substr((mid_prefix + "resnets.0.").size());
            replace_with_name_map(name, block_map);
            return name;
        }
        if (starts_with(name, mid_prefix + "attentions.0.")) {
            name = std::string(scope) + ".middle.1." + name.substr((mid_prefix + "attentions.0.").size());
            return name;
        }
        if (starts_with(name, mid_prefix + "resnets.1.")) {
            name = std::string(scope) + ".middle.2." + name.substr((mid_prefix + "resnets.1.").size());
            replace_with_name_map(name, block_map);
            return name;
        }
    }

    const std::string decoder_up_prefix = "decoder.up_blocks.";
    if (starts_with(name, decoder_up_prefix)) {
        for (int i = 0; i < 4; ++i) {
            const std::string block_prefix = decoder_up_prefix + std::to_string(i) + ".";
            for (int j = 0; j < 3; ++j) {
                const std::string resnet_prefix = block_prefix + "resnets." + std::to_string(j) + ".";
                if (starts_with(name, resnet_prefix)) {
                    const int flat_index = i * 4 + j;
                    name = "decoder.upsamples." + std::to_string(flat_index) + "." + name.substr(resnet_prefix.size());
                    replace_with_name_map(name, block_map);
                    return name;
                }
            }
            ...
            if (i < 3) {
                const std::string upsampler_prefix = block_prefix + "upsamplers.0.";
                if (starts_with(name, upsampler_prefix)) {
                    const int flat_index = i * 4 + 3;
                    return "decoder.upsamples." + std::to_string(flat_index) + "." + name.substr(upsampler_prefix.size());
                }
            }
        }
    }

    const std::string encoder_down_prefix = "encoder.downsamples.";
    if (starts_with(name, encoder_down_prefix)) {
        replace_with_name_map(name, block_map);
    }
