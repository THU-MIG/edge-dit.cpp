...
// SageAttention2 "Thorough Outlier Smoothing" rank-1 compensation for smooth-Q.
// When Q is smoothed by subtracting its per-channel mean qm before INT8
// quantization, the kernel computes (Q-qm)·(K-km) instead of Q·K. Expanding:
//   (Q-qm)·(K-km) = Q·K - Q·km - qm·K + qm·km
// The -Q·km term is per-query-row and +qm·km is a global constant -- both are
// invariant under the row-wise softmax and can be dropped. The -qm·K[j] term is
// PER-KEY and must be added back, otherwise the score row is distorted (this is
// exactly the FLUX quality collapse). QK_bias[j] = qm·K_orig[j] is precomputed
// on the host (one value per key, per head-batch). RS here is in dequantized
// score units (i.e. already multiplied by q_scale*k_scale), matching the units
// of QK_bias, so the bias is a plain additive term before the softmax scaling.
// The key-index mapping mirrors apply_out_of_bound_mask exactly.
template <uint32_t num_tiles_q, uint32_t num_tiles_k>
__device__ __forceinline__ void add_smooth_q_bias(float RS[][num_tiles_k][8],
...
#pragma unroll
    ...
    {
      const uint32_t kv_idx = K_idx_lane_base + fk * 16 + 8 * (k / 4) + k % 2;
      const float b = (kv_idx < kv_len) ? bias_row[kv_idx] : 0.0f;
#pragma unroll
      for (uint32_t fq = 0; fq < num_tiles_q; fq++)
      {
        RS[fq][fk][k] += b;
