...
- **diffusers** (optional, Python reference): needs a Python with `torch`/`diffusers`/`transformers` installed; the site's `diffusers_python` points to it. Quantization uses **Optimum-Quanto**: `bf16` (baseline) / `fp8` (qfloat8 weight-only, for flux/qwen) / `w8` (qint8 weight-only, for SD3/SD3.5-turbo — qfloat8 is too coarse for 2B MMDiT); **no q4**; CUDA only.
