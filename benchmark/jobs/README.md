...
| `warmup` | | `1` | `N` untimed warmup generations **per config** (one config = one model×quant×offload×cache row), to absorb cold-start (kernel JIT, cuDNN autotune, first alloc). Semantics: warmup runs **once per config, on that config's first prompt only** — all `prompts` (including the first) are still fully measured, so a config = 1 warmup pass + N measured prompts. When `warmup: N`, all N warmup passes run on the **first prompt** (not one per prompt). Their latency goes to `warmup_ms`, **excluded** from reported speed/quality; `0` disables warmup. All three systems honor it via `--warmup-runs`. For edge-dit, all three tasks (t2i / edit / video) go through the same `ed-sample` path (load once, then N+1 untimed→timed passes with the model resident), so the measurement boundary is identical across tasks: model load is excluded, generation is timed tightly, output encoding is excluded. |
