...
- **diffusers** (optional, Python reference): needs a Python with `torch`/`diffusers`/`transformers` installed; the site's `diffusers_python` points to it. Quantization uses **Optimum-Quanto**: `bf16` (baseline) / `w8` (qint8 weight-only, for all diffusers models — mirrors edge/sd.cpp q8_0); **no q4**; CUDA only.
