...
bool QwenImagePipeline::build_components(const ed_context_params_t& params,
    ...
    // sizing (vs the fixed 4 GiB constant). Build a temporary QwenImageRunner spec (shapes
    // only, no weights) to run the measure. No-op unless auto_allocate + a target size;
    // 0 measured -> caller falls back to fixed headroom.
